...
static uint8_t dma_rx_buffer[17];
...
void ElevatorMPU_Init(void)
{
  ...
  __HAL_UART_ENABLE_IT(&huart1, UART_IT_IDLE);	
    */
    ...
    /* 配置USART1 DMA循环接收 */
    if (HAL_UART_Receive_DMA(&huart1, dma_rx_buffer, 17) != HAL_OK)
    {
...
void ElevatorMPU_PrepareResponse(void)
{
    ...
    /* 应答命令（回显接收到的命令） */
    elevator_tx_data.ack_command = 2;
    elevator_tx_data.sensor_communicate_status = 1;

    
    /* 填充其他参数 */
    elevator_tx_data.ack_command_d2 = 0x00;  // 根据实际需求设置
    elevator_tx_data.sensor_communicate_status_d2 = 0x00;
	
    elevator_tx_data.stayed_person = 0x00;
	
//    elevator_tx_data.reserved = 0x00;
