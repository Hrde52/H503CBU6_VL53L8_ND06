...
int example8(void)
{
	...
	*/
	...
	status = vl53l8cx_calibrate_xtalk(&Dev, 88, 4, 500);
	if(status)
	{
	...
	}else
	{
		...
		status = vl53l8cx_get_caldata_xtalk(&Dev, xtalk_data);
		  ...
		  printf("store the data into the flash!\n");
		  if (Flash_Write_Array((uint8_t*)&xtalk_data, sizeof(xtalk_data)) == HAL_OK)
		    {
