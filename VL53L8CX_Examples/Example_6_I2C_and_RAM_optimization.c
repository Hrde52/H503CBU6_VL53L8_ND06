...
int example6(void)
{
	...
	uint8_t 				resolution, status, loop, isAlive, isReady, i;
	...
	*/
	...
	status = vl53l8cx_set_ranging_frequency_hz(&Dev, 10);// Set 10Hz ranging frequency
	...
	while(loop < 10000)
	{
		...
		if(isReady)
		{
			...
			{
				printf("Zone : %3d, Status : %3u, Distance : %4d mm, Re : %4d\n",
					i,
					Results.target_status[VL53L8CX_NB_TARGET_PER_ZONE*i],
					Results.distance_mm[VL53L8CX_NB_TARGET_PER_ZONE*i],
					Results.reflectance[VL53L8CX_NB_TARGET_PER_ZONE*i]);
			}
			printf("\n");
			loop++;
		}
